...
int main() {
    ...
    // 创建 SURF 检测器（需要 NONFREE 已启用）
    Ptr<SURF> surf = SURF::create(400, 4, 3, true, false);
    ...
    t_det1.start();
    surf->detect(img1, key1);
    t_det1.stop();

    t_desc1.start();
    surf->compute(img1, key1, des1);
    t_desc1.stop();

    // 图2：同样分开计时
    t_det2.start();
    surf->detect(img2, key2);
    t_det2.stop();

    t_desc2.start();
    surf->compute(img2, key2, des2);
    t_desc2.stop();

    // 打印关键点数与时间
    std::cout << "[SURF] #kp1=" << key1.size() << "  #kp2=" << key2.size() << "\n";
    std::cout << "[SURF] detect(ms): img1=" << t_det1.getTimeMilli()
        << "  img2=" << t_det2.getTimeMilli() << "\n";
    std::cout << "[SURF] describe(ms): img1=" << t_desc1.getTimeMilli()
    ...
    t_ratio.stop();
    std::cout << "[SURF] match(ms)=" << t_match.getTimeMilli()
    ...
    if (good.size() >= 4) {
        ...
        std::cout << "[SURF] inliers=" << inliers
            << "  inlier_ratio=" << inlier_ratio
            << "  reproj_err(px)=" << reproj_err << std::endl;
    }
    else {
        std::cout << "[SURF] Not enough matches for RANSAC" << std::endl;
    }
    ...
    std::cout << "[SURF] descriptor_len=" << bytes_per_desc << " bytes"
    ...
    // 3) 窗口可缩放
    cv::namedWindow("SURF Matches", cv::WINDOW_NORMAL);
    ...
    if (vis.cols > targetW) {
        ...
        cv::imshow("SURF Matches", vis_small);
        cv::imwrite("matches.jpg", vis_small);
    }
    else {
        cv::imshow("SURF Matches", vis);
