...
#include <filesystem>
using namespace cv;
using namespace cv::xfeatures2d;   //nfeatures=1000起步，图像分辨率大就1500-2000
...
int main() {
    ...
    // 创建 ORB 检测器（需要 NONFREE 已启用）
    cv::Ptr<cv::Feature2D> f2d = cv::ORB::create(
        /*nfeatures=*/10000, /*scaleFactor=*/1.2f, /*nlevels=*/8,
    ...
    // 打印关键点数与时间
    std::cout << "[ORB] #kp1=" << key1.size() << "  #kp2=" << key2.size() << "\n";
    std::cout << "[ORB] detect(ms): img1=" << t_det1.getTimeMilli()
        << "  img2=" << t_det2.getTimeMilli() << "\n";
    std::cout << "[ORB] describe(ms): img1=" << t_desc1.getTimeMilli()
    ...
    t_ratio.stop();
    std::cout << "[ORB] match(ms)=" << t_match.getTimeMilli()
    ...
    if (good.size() >= 4) {
        ...
        std::cout << "[ORB] inliers=" << inliers
            << "  inlier_ratio=" << inlier_ratio
            << "  reproj_err(px)=" << reproj_err << std::endl;
    }
    else {
        std::cout << "[ORB] Not enough matches for RANSAC" << std::endl;
    }
    ...
    std::cout << "[ORB] descriptor_len=" << bytes_per_desc << " bytes"
    ...
    if (vis.cols > targetW) {
        ...
        cv::imshow("ORB Matches", vis_small);
        cv::imwrite("matches.jpg", vis_small);
    }
    else {
        cv::imshow("ORB Matches", vis);
