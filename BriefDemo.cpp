...
int main() {
    ...
    // 创建 BRIEF 检测器（需要 NONFREE 已启用）
// 关键点检测器（FAST）
    cv::Ptr<cv::Feature2D> kdet = cv::FastFeatureDetector::create(
        /*threshold=*/20, /*nonmaxSuppression=*/true
    );

    // BRIEF 描述子（bytes 可选 16/32/64；32=256bit）
    cv::Ptr<cv::xfeatures2d::BriefDescriptorExtractor> f2d =
        cv::xfeatures2d::BriefDescriptorExtractor::create(/*bytes=*/32, /*use_orientation=*/false);
    ...
    // 打印关键点数与时间
    std::cout << "[BRIEF] #kp1=" << key1.size() << "  #kp2=" << key2.size() << "\n";
    std::cout << "[BRIEF] detect(ms): img1=" << t_det1.getTimeMilli()
        << "  img2=" << t_det2.getTimeMilli() << "\n";
    std::cout << "[BRIEF] describe(ms): img1=" << t_desc1.getTimeMilli()
    ...
    // 匹配计时
    t_match.start();
    cv::BFMatcher matcher(cv::NORM_HAMMING);   // BRIEF 用汉明距离
    ...
    t_ratio.stop();
    std::cout << "[BRIEF] match(ms)=" << t_match.getTimeMilli()
    ...
    if (good.size() >= 4) {
        ...
        std::cout << "[BRIEF] inliers=" << inliers
            << "  inlier_ratio=" << inlier_ratio
            << "  reproj_err(px)=" << reproj_err << std::endl;
    }
    else {
        std::cout << "[BRIEF] Not enough matches for RANSAC" << std::endl;
    }
    ...
    std::cout << "[BRIEF] descriptor_len=" << bytes_per_desc << " bytes"
    ...
    // 3) 窗口可缩放
    cv::namedWindow("BRIEF Matches", cv::WINDOW_NORMAL);
    ...
    if (vis.cols > targetW) {
        ...
        cv::imshow("BRIEF Matches", vis_small);
        cv::imwrite("matches.jpg", vis_small);
    }
    else {
        cv::imshow("BRIEF Matches", vis);
